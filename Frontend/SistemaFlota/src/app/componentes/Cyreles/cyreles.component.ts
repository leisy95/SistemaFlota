...
export class CyrelesComponent implements OnInit {
  ...
  cargarCajones() {
    ...
    this.svc.getCajones().subscribe({
      next: (data: any) => { this.cajones = data; this.cargando = false; this.cdr.markForCheck(); },
  ...
  aplicarFiltro() {
    ...
    this.registrosFiltrados = !q
      ? [...this.registros]
      : this.registros.filter(r => r.nombre.toLowerCase().includes(q));
    this.cdr.markForCheck();
  }

  // ── Modales ────────────────────────────────────────────────────────────────
  abrirCrearCajon() {
    this.formCajon = { numero: 0, descripcion: '' };
    this.modal = 'crear-cajon';
    this.cdr.markForCheck();
  }
  ...
  guardarCajon() {
    ...
    obs.subscribe({
      next: () => {
        ...
        this.cerrarModal();
        this.mostrarExito(this.modal === 'crear-cajon' ? 'Cajón creado' : 'Cajón actualizado');
        this.cargarCajones();
      },
      error: (err: any) => {
        this.guardando = false;
        this.mostrarError(err?.error ?? 'Error al guardar cajón');
        this.cdr.markForCheck();
      }
    });
  }
  ...
  guardarRegistro() {
    ...
    obs.subscribe({
      next: () => {
        this.guardando = false;
        this.cerrarModal();
        this.mostrarExito(this.modal === 'crear-registro' ? 'Registro creado' : 'Registro actualizado');
        this.cargarRegistros(this.cajonActual.id);
      },
  ...
  eliminarRegistro(r: any) {
    ...
    this.svc.eliminarRegistro(r.id).subscribe({
      next: () => {
        ...
        if (this.vista === 'detalle') this.volverARegistros();
        this.cargarRegistros(this.cajonActual.id);
      },
      error: () => this.mostrarError('Error al eliminar')
    });
  ...
  exportarExcel() {
    const datos = this.registrosFiltrados.map((r, i) => ({
      'N°':          i + 1,
      'Cajón':       `Cajón ${r.cajonNumero}`,
      'Nombre':      r.nombre,
      'Fecha':       new Date(r.fechaCreacion).toLocaleDateString('es-CO'),
    }));
  ...
  exportarPDF() {
    ...
    autoTable(doc, {
      ...
      body: this.registrosFiltrados.map((r, i) => [
        i + 1,
        r.nombre,
