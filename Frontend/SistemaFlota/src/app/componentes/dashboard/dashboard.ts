...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  cargarDatos() {
    ...
    this.inspeccionesService.obtenerTotalInspecciones().subscribe({
      next: (data) => { this.totalInspecciones = data.length; this.ultimasInspecciones = data.slice(0, 5); },
      error: (err)  => console.error(err)
    });
    ...
    this.incidentesService.obtenerIncidentes().subscribe({
      next: (data: any[]) => this.incidentesPendientes = data.filter(i => i.estado === 'Pendiente').length,
      error: (err) => console.error(err)
    });
    this.mantenimientoService.obtenerTodos().subscribe({
      next: (data: any[]) => this.vehiculosEnTaller = data.filter(m => m.estado === 'EnTaller').length,
      error: (err) => console.error(err)
    });
